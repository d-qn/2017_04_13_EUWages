...
dd3 %<>% mutate(val = (((values - wage.lt30) / wage.lt30)  * 100)) 
...
dd3.gp <- ggplot(dd3 %>% filter(isco08 != "TOTAL")) + 
  geom_line(aes(x = age, y = val, group = geo, colour = geo, size = size)) +
...
# ddd <- dd3 %>% filter(isco08 == "OC1")
# # http://stackoverflow.com/questions/10667995/line-chart-highcharts
# 
# 
# hchart(ddd, "spline",
#              hcaes(x = age, y = val, group = geo, value = values, tooltip = values)) %>% hc_add_theme(hc_theme_swi)
# 
# highchart() %>% hc_add_series(data = ddd, type = "line")
# 
# %>%
#   hc_plotOptions(series = list(marker = list(radius = 2),
#                   dataLabels = list(enabled = T, formatter = JS('function () {return this.series.name;}'), x = 5
#                   )))
